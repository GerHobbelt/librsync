...
const uint32_t RS_MD4_SUM_LENGTH = 16;
const uint32_t RS_BLAKE2_SUM_LENGTH = 32;

static void rs_block_sig_init(rs_block_sig_t *sig, rs_weak_sum_t weak_sum,
                              rs_strong_sum_t *strong_sum, uint32_t strong_len)
{
...
static inline uint32_t rs_block_sig_size(const rs_signature_t *sig)
{
    /* Round up to next multiple of sizeof(weak_sum) to align memory correctly.
     */
    return (uint32_t) (offsetof(rs_block_sig_t, strong_sum) +
                       ((sig->strong_sum_len + sizeof(rs_weak_sum_t) - 1) /
...
static inline uint32_t rs_block_sig_idx(const rs_signature_t *sig,
                                      rs_block_sig_t *block_sig)
{
    return (uint32_t) ((char *)block_sig -
                            ...
                            uint32_t strong_len, rs_long_t sig_fsize)
{
    ...
    sig->size = (uint32_t) (sig_fsize ? (sig_fsize - 12) / (4 + strong_len) : 0);
    if (sig->size)
        sig->block_sigs =
            rs_alloc((size_t) sig->size * rs_block_sig_size(sig),
                                       ...
                                       rs_strong_sum_t *strong_sum)
{
    ...
    if (sig->count == sig->size) {
        ...
        sig->block_sigs =
            rs_realloc(sig->block_sigs, (size_t) sig->size * rs_block_sig_size(sig),
                       "signature->block_sigs");
    }
...
void rs_signature_log_stats(rs_signature_t const *sig)
{
    ...
    rs_log(RS_LOG_INFO | RS_LOG_NONAME,
           ...
           100.0 * (double)t->match_count / (double)t->find_count, t->hashcmp_count,
           (double)t->hashcmp_count / (double)t->find_count, t->entrycmp_count,
...
rs_result rs_build_hash_table(rs_signature_t *sig)
{
    ...
    rs_signature_check(sig);
    sig->hashtable = hashtable_new((size_t) sig->count);
    if (!sig->hashtable)
        return RS_MEM_ERROR;
    for (i = 0; i < sig->count; i++) {
        b = rs_block_sig_ptr(sig, i);
        rs_block_match_init(&m, sig, b->weak_sum, &b->strong_sum, NULL, (size_t) 0);
...
void rs_sumset_dump(rs_signature_t const *sums)
{
    ...
    rs_log(RS_LOG_INFO | RS_LOG_NONAME,
           "sumset info: magic=%#x, block_len=%u, block_num=%ju", sums->magic,
           sums->block_len, (uintmax_t) sums->count);

    for (i = 0; i < sums->count; i++) {
        b = rs_block_sig_ptr(sums, i);
        rs_hexify(strong_hex, b->strong_sum, (size_t) sums->strong_sum_len);
        rs_log(RS_LOG_INFO | RS_LOG_NONAME,
               "sum %6d: weak=%ju, strong=%s", i, (uintmax_t) b->weak_sum,
               strong_hex);
    }
}
